        ...
        try:
            ...
            if res.status_code == 200:
                ...
                for elem in today_weather:
                    ...
                    forecast_dict['max-diff'] = elem.select_one('.high-temp.tempdiff').getText()
                    forecast_dict['min'] = elem.select_one('.low-temp.temp .value').getText()
                    forecast_dict['min-diff'] = elem.select_one('.low-temp.tempdiff').getText()
                    forecast_dict['icon'] = elem.select_one('.weather-icon img').get('src')
                    forecast_dict['rainy-percent'] = [percent.getText() for percent in elem.select('.rain-probability td')]
            else:
